...
function boardItems(status) {
  ...
  return boardAll(status).filter(function (it) {
    if (it.src === 'task') {
...
function renderTareasLista(body) {
  ...
  // Lo que ya está en marcha primero, luego lo pendiente y al final lo hecho:
  // la lista responde a "¿qué estoy haciendo ahora?" antes que a "¿qué me queda?".
  ...
  tasks.sort(function (a, b) {
    ...
    return (rank[sa] - rank[sb]) || (a.order - b.order);
  });
...
function taskRow(t) {
  ...
  var row = h('div', { class: 'planner-task' + (t.done ? ' done' : ''), 'data-id': t.id },
    h('div', { class: 'planner-task-row' },
      ...
      h('button', { class: 'act', title: open ? 'Ocultar los pasos' : 'Ver/añadir los pasos para completarla', onclick: function () { tareasOpen[t.id] = !open; renderTareas(); } }, icon(open ? 'chevronDown' : 'chevron')),
...
function blockRow(b) {
  ...
  var row = h('div', { class: 'planner-task from-canvas' + (b.kanban === 'done' ? ' done' : ''), 'data-id': b.id },
    h('div', { class: 'planner-task-row' },
      chk, title,
      b.subs.length ? h('span', { class: 'planner-subcount' + (subsDone === b.subs.length ? ' all' : '') }, subsDone + '/' + b.subs.length) : null,
      h('button', { class: 'act', title: open ? 'Ocultar los pasos' : 'Ver/añadir los pasos', onclick: function () { tareasOpen[b.id] = !open; renderTareas(); } }, icon(open ? 'chevronDown' : 'chevron')),
...
function boardCard(it, status) {
  ...
  var card = h('div', {
    class: 'kanban-card' + (!isTask && ref.important ? ' important' : '') + (!isTask && ref.blocked ? ' blocked' : '') + (isTask ? ' is-task' : ' is-block'),
  ...
  var actions = h('div', { class: 'kc-actions' },
    ...
    h('button', { class: 'kc-btn' + (open ? ' on' : '') + (subs.length ? ' has-steps' : ''), title: open ? 'Ocultar los pasos' : 'Ver/añadir los pasos', onclick: function () { tareasOpen[it.id] = !open; renderTareas(); } }, icon('todo'), subs.length ? h('span', { class: 'kc-steps-n' }, subsDone + '/' + subs.length) : null),
    isTask
